...
C_TEXT:C284($kTxt_fontFamily;$kTxt_reportObject;$Txt_;$Txt_data;$Txt_format;$Txt_object)
...
If (True:C214)  // 1st column
	...
	$tPtr_columns{$Lon_column}->{$Lon_row}:=""
	  ...
	  //rows data-source {
	$Lon_row:=2
	QR GET INFO COLUMN:C766($Lon_area;$Lon_row;$Txt_;$Txt_object;$Lon_;$Lon_width;$Lon_;$Txt_format)
	
	If (Length:C16($Txt_object)>0)
		
		$Txt_object:=db_virtualFieldName ($Txt_object)
		$tPtr_columns{$Lon_column}->{$Lon_row}:=report_cell_styled_content ($Lon_area;$Txt_object;$Lon_column;$Lon_row;$tTxt_columns{$Lon_column})
...
If (True:C214)  // 2nd column
	...
	$Lon_row:=2
	  ...
	  //beware: column = 3 !
	QR GET INFO COLUMN:C766($Lon_area;3;$Txt_;$Txt_object;$Lon_;$Lon_;$Lon_;$Txt_format)
	...
	If (Length:C16($Txt_object)>0)
		...
		Else 
			...
			$tPtr_columns{$Lon_column}->{$Lon_row}:=report_cell_styled_content ($Lon_area;"<span>"+"\r"+$Txt_object+"\r\r</span>"\
